...
LEFTOVER_EXCLUDE_DOMAINS = {"linkedin.com", "cn.linkedin.com", "www.linkedin.com", "x.com", "twitter.com", "facebook.com", "www.facebook.com", "instagram.com", "www.instagram.com", "tiktok.com", "www.tiktok.com", "b23.tv", "weibo.com", "www.weibo.com", "youtube.com", "www.youtube.com", "m.youtube.com"}
# 导航/栏目录页判定（任意段命中即剔；article/column/trends/story/pd/view 等承载段不在表内，天然放行）
LEFTOVER_NAV_SEG = {"articles", "category", "categories", "channel", "channels", "video", "videos", "people", "company", "companies", "home", "archives", "archive", "posts", "about", "about-us", "uk", "us", "news", "technology", "topics", "topic", "section", "sections", "list", "lists", "tag", "tags", "noticias", "mag", "magazine", "content", "p", "page"}
# 单段 path 仍然可能是文章页的站（其余单段一律按栏目/主页剔）
LEFTOVER_SINGLE_SEG_OK = {"telegra.ph", "solidot.org", "linux.solidot.org"}

def pick_leftover_item(items, dedup, kept_urls, window_start, today):
    ...
    for it in items:
        ...
        if pub and pub > today_s:
            continue
        if pub and not month_only and pub < window_start_s:
            try:
            ...
            if outside > 7:
                continue
        pool.append({"title": title[:200], "url": url, "domain": domain,
                     "publish_date": pub or "", "date_verified": verified or bool(pub)})
    if not pool:
        return None
...
def build_source_leftovers(src_map, kept, dedup, window_start, today):
    ...
    for key, info in src_map.items():
        ...
        if not picked:
            continue
        leftovers.append({"source": info["label"], "source_key": key,
                          "direction": info["direction"], **picked})
    return leftovers
...
def _main():
    ...
    for c in candidates:
        ...
        c["source_key"] = "hn"; c["source_label"] = "Hacker News"
    ...
    if extra: rank.search_batch_with_tags("pref", extra, candidates, src_stats, pref_dir, src_map)
    # Telegram 频道采集
    ...
    for c in tg_items:
        ...
        c["source_key"] = "tg:" + ch; c["source_label"] = "TG@" + ch
    ...
    payload = {"generated_at": now.isoformat(), "window": {"start": str(window_start), "end": str(today)}, "preference": {"pref_dir": pref_dir, "explore_dir": explore_dir, "likes_count": len(rank.likes_mod.load_likes(LIKES_FILE))}, "candidates": kept, "source_leftovers": leftovers}
